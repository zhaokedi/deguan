...
function logResult1($word = '')
{
    $mystrs = str_replace("ThinkPHP/Common", "", dirname(__FILE__));
    $mystrs = str_replace("ThinkPHP\\Common", "", $mystrs);
    //  $myppath="C:\\Users\\Administrator\\Desktop\\log\\".date('Y-m-d')."log.txt";
    $savepathfull = $mystrs . "log/";
    if (!file_exists($savepathfull)) {
        $balue = mkdir($savepathfull, 777, true);
        if ($balue == false) {
            return false;
        }
    }
    ...
    $savepathfull = $savepathfull . date('Y-m-d') . "log.txt";
    $text = "执行日期：" . strftime("%Y-%m-%d %H:%M:%S ", time()) . "\r\n" . $word . "\r\n结束";
    return file_put_contents($savepathfull, $text, FILE_APPEND);


}
function logResult2($word = ''){

      $myppath=dirname(dirname(dirname(__FILE__)));
    $savepathfull = $myppath . "/log/";
